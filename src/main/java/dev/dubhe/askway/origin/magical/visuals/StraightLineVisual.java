...
public class StraightLineVisual implements IVisual {
    ...
    public void display(ICaster caster, AbstractElement element, int energy, ITarget target) {
        if (!(target.getLevel() instanceof ServerLevel level)) return;
        ...
        for (int i = 0; i < energy; i++) {
            ...
            level.sendParticles(new DustParticleOptions(element.getColor().getVec3f(), element.getColor().getAlphaF()), pos.x, pos.y, pos.z, 1, 0, 0, 0, 0);
        }
    }
